...
void add_client(struct sockaddr_in in, uint32_t hash) {
...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        if (buffer->info.flags == 1) {
            ...
            add_client(*(data->client_addr), buffer->sender); 
            ...
            if (buffer->target == 0) {
                ...
                if (target) sendto(serverFD, (char*)(buffer->data), buffer->info.len, 0, (struct sockaddr*)target, sizeof(*target));
                pthread_mutex_unlock(&clients_lock);
            }
        }
        free(buffer);
